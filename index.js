...
(function drawStars() {
  ...
  stars.forEach(s => {
    ...
    const opacity =
      0.3 + 0.7 * Math.abs(Math.sin(s.a));

    sctx.beginPath();
    sctx.arc(s.x, s.y, s.r, 0, Math.PI * 2);

    sctx.fillStyle =
      `rgba(255,220,240,${opacity})`;
  ...
  setTimeout(() => {
    ...
    if (depth > 3 && Math.random() > 0.5) {
      drawBranch(
        x2,
        y2,
        angle - spread * 0.4 + Math.random() * 0.2,
        length * 0.6,
        depth - 1,
        delay + 80
      );
...
function bloomFlowers() {
  ...
  setTimeout(
    showMessages,
    blooms.length > 0
      ? blooms[blooms.length - 1].delay + 600
      : 800
  );
}
